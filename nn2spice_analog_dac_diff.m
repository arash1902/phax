 function [v, delay, power, energy] = nn2spice_analog_dac_diff(filename, data_in, num_proc,  varargin)
...
fprintf(fid, '\n*===============================================');
%  =============== Make DAC interface ======================
% BITS_PER_ANALOG_IN = floor(nbits/(layers(1)/2));
BITS_PER_ANALOG_IN =nbits/nanalogs;
MAX_PARALLEL = 16;
Rmin = 128e3;
for idac = 1:layers(1) %freach analog input
    fprintf(fid, '\n *-------------- A%d ------------', idac);
    for jdac = 1:BITS_PER_ANALOG_IN 
        fprintf(fid, '\n r_dacp_%d_%d  Dp%d inp1_%d %3.2e',idac, jdac, jdac-1+(idac-1)*BITS_PER_ANALOG_IN, idac, (Rmin/MAX_PARALLEL)*2^(BITS_PER_ANALOG_IN-jdac));
        fprintf(fid, '\n r_dacn_%d_%d  Dn%d inn1_%d %3.2e',idac, jdac, jdac-1+(idac-1)*BITS_PER_ANALOG_IN, idac, (Rmin/MAX_PARALLEL)*2^(BITS_PER_ANALOG_IN-jdac));
    end
   % fprintf(fid, '.prot\n.include ''comp.sp''\n.unprot\n');
%     fprintf(fid, '\nxdacinv%d	inp1_%d	inn1_%d	 vdd_logic vss inv_diff sz=5\n',  idac, idac, idac);
end
...
% note that here data_in is a digital vector
for i=1:Nsamples
    for jj=1:Nin*BITS_PER_ANALOG_IN
       % bin_vec = binary2vector(floor(data_in(jj,i)*(2^BITS_PER_ANALOG_IN-1)),BITS_PER_ANALOG_IN);
        bin_vec = data_in(jj,i)-~data_in(jj,i);
        fprintf(fid2, ' %3.2e',bin_vec*vdd_logic/2);
    end
    fprintf(fid2, '\n');
end
fclose(fid2);
...
[~, Nin] = size(weights{1});
Nin = Nin./2;
for ki = 1:Nin*BITS_PER_ANALOG_IN
       fprintf(fid, ' xvd%d=%d', ki, ki);
end
fprintf(fid, '\n.ENDDATA\n');
...
% for ki = 1:Nin*BITS_PER_ANALOG_IN
% %        fprintf(fid, '\n .param xv%d=0', ki);
%        fprintf(fid, '\n .param xvd%d=''xv%d''', ki, ki);
%        %fprintf(fid, '\n .param xvn%d=''-xv%d''', ki, ki);
% end
% =================== input sources ==================================
fprintf(fid, '\n vb0 b0 0 ''-parvdd_logic/2'' \n');
fprintf(fid, '\n vb1 b1 0 ''parvdd_logic/2'' \n'); %vdd_logic = 1V
for iii=1:Nin*BITS_PER_ANALOG_IN
      fprintf(fid, '\nvxdp%d Dp%d 0 ''xvd%d''', iii, iii-1, iii);
      fprintf(fid, '\nvxdn%d Dn%d 0 ''-xvd%d''', iii, iii-1, iii);
%     fprintf(fid, '\nvxn%d inn1_%d 0 ''xvn%d''', iii, iii, iii);
end
% ========================= measure analog inputs for debugging
for iii=1:Nin
    fprintf(fid, '\n.MEAS TRAN in_analogp%d AVG v(inp1_%d) from=99.9999n to=100n', iii, iii);
    fprintf(fid, '\n.MEAS TRAN in_analogn%d AVG v(inn1_%d) from=99.9999n to=100n', iii, iii);
end
